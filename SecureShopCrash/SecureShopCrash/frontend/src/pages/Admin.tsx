...
const TABS: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  ...
  { key: 'products', label: 'Mỹ phẩm', icon: <Package className="w-5 h-5" /> },
  { key: 'categories', label: 'Danh mục', icon: <FolderTree className="w-5 h-5" /> },
  { key: 'brands', label: 'Nhãn hàng', icon: <Tag className="w-5 h-5" /> },
  ...
  { key: 'discount', label: 'Ưu đãi', icon: <Percent className="w-5 h-5" /> },
  { key: 'users', label: 'Khách hàng', icon: <Users className="w-5 h-5" /> },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="bg-gradient-to-r from-rose-500 to-fuchsia-600 rounded-2xl shadow-lg p-8 text-white">
            <h1 className="text-3xl sm:text-4xl font-bold mb-2">Quản trị Lumière Beauty</h1>
            <p className="text-purple-100 text-lg">
              Quản lý toàn bộ sản phẩm, đơn hàng và nội dung cho cửa hàng mỹ phẩm
            </p>
          </div>
        </div>
                ...
                {TABS.map((tab) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-200 ${
                      activeTab === tab.key
                        ? 'bg-gradient-to-r from-rose-500 to-fuchsia-600 text-white shadow-md transform scale-[1.02]'
