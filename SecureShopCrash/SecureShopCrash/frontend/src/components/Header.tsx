...
const Header: React.FC = () => {
  ...
  const popularSearches = ["Son môi", "Serum", "Kem chống nắng", "Mặt nạ dưỡng da"];
  ...
  return (
    <header className="bg-white/90 backdrop-blur-md shadow-md sticky top-0 z-50 w-full border-b border-rose-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2 flex-shrink-0">
            <Sparkles className="h-7 w-7 sm:h-8 sm:w-8 text-rose-500" />
            <span className="text-lg sm:text-xl font-bold text-zinc-800">Lumière Beauty</span>
          </Link>
                  ...
                  >
                    <form onSubmit={handleSearchSubmit} className="relative">
                      <input
                        ref={searchInputRef}
                        type="text"
                        placeholder="Tìm kiếm mỹ phẩm, serum, son môi..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full pl-10 pr-10 py-2.5 border-2 border-rose-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all"
                      />
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-rose-500 h-5 w-5 pointer-events-none" />
                      <button
                        type="button"
